...
install: 
	...
	@m4  -P -DSERVICE_PROTOCOL=$(SERVICE_PROTOCOL) \
					...
					$(RESOURCES_DIR)/config.toml.m4 > $(ARTIFACTS_DIR)/config.toml

	ln $(ARTIFACTS_DIR)/config.toml $(INSTALL_DIR)/src/config/config.toml
...
build: 
	...
	@m4  -P -DSERVICE_PROTOCOL=$(SERVICE_PROTOCOL) \
					...
					$(RESOURCES_DIR)/config.toml.m4 > $(ARTIFACTS_DIR)/config_build.toml
